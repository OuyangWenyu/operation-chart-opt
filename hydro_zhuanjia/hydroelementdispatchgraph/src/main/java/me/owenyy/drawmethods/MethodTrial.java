...
/**
 * 分区采用试算法，分区的代码还没写好，因此先采用固定供蓄水期的方式
 * 
 * @author OwenYY
 *
 */
public class MethodTrial implements DispatchGraphBehavior {
	...
	public void makeDispatchGraph(PowerControlHStation hStation, DispatchInputParas input) {
		...
		TimeSeqCurve runoffData = tscm.createTimeSeqCurve(hsSpec.getId() / 10 * 10, 1000, tbType,
				hStation.getHydroStation().getHsStates().size(),hStation.getHydroStation().getHsStates().get(0).getTimeStart(),hStation.getHydroStation().getHsStates().get(hStation.getHydroStation().getHsStates().size()-1).getTimeEnd());
		if(runoffData==null){
			int nums=hStation.getHydroStation().getHsStates().size();
			double[] data=new double[nums];
			for(int i=0;i<nums;i++){
				data[i]=hStation.getHydroStation().getHsStates().get(i).getInflowReal();
			}
			TimeSeqCurveFactory timeSeqCurveFactory = new TimeSeqCurveFactory(tbType, 1, hStation.getHydroStation().getHsStates().get(0).getTimeStart(), nums, data);
			runoffData = timeSeqCurveFactory.curvePlotting();
		}
		MakeSureByOutput divideDeliveryAndStorage = new MakeSureByOutput();
		divideDeliveryAndStorage.setServeTime(new ServeTime());
				//new FixedDividePeriod(hStation.getHsStates().get(0).getTimeStart().getYear(), periodstart));
		...
		if(hsSpec.getLevelFloodControl()>0){
			int floodstartindex=0;
			int floodendindex=0;
			for(int i=0;i<hStation.getHydroStation().getHsStates().size();i++){
				if(hStation.getHydroStation().getHsStates().get(i).getLevelMax()==hStation.getHydroStation().getHsSpec().getLevelFloodControl())
				{
					floodstartindex=i;
					break;
				}
			...
			for(int i=0;i<hStation.getHydroStation().getHsStates().size();i++){
				if(i>0 && hStation.getHydroStation().getHsStates().get(i-1).getLevelMax()==hStation.getHydroStation().getHsSpec().getLevelFloodControl()
						&& hStation.getHydroStation().getHsStates().get(i).getLevelMax()>hStation.getHydroStation().getHsSpec().getLevelFloodControl())
				{
					floodendindex=i;
					break;
				}
			}
			LocalDate floodControlBegin=hStation.getHydroStation().getHsStates().get(floodstartindex).getTimeStart().toLocalDate();
			divideDeliveryAndStorage.setFloodControlBegin(floodControlBegin);
			LocalDate floodControlEnd=hStation.getHydroStation().getHsStates().get(floodendindex).getTimeStart().toLocalDate();;
			divideDeliveryAndStorage.setFloodControlEnd(floodControlEnd);
		}
		MultiFomulaForDispatch1 mffd1 = new MultiFomulaForDispatch1();
		mffd1.setCurve(hStation.getHydroStation().getStationCurves());
		mffd1.setFixedHeadLoss(1);
		mffd1.setHsSpec(hsSpec);
		divideDeliveryAndStorage.setMfm(mffd1);
		...
		DivideDeliveryAndStorage serveTime =new FixedDividePeriod(
				hStation.getHydroStation().getHsStates().get(0).getTimeStart().getYear(), periodstart); /*new ServeTime();*/
		...
		regulationYears = generateRegulationYear.generateRegulationYearsBeginWithSetvalue(runoffData, hsSpec,
		 ...
		 double warrantedOutputSearch =
		  divideDeliveryAndStorage.getGuaranteedOutputSearch();
		 hsSpec.setOutputGuaranteed(warrantedOutputSearch);//12-4月计算的值：95%-169.26061785057237
		...
		double[][] storagePeriodWarrentedOutputLine = drawDispatchLine.drawStorageLine(constraintTypes,
				sneighborRegulationYears, scorrectionFactors,
				hsSpec.getOutputGuaranteed()/*
											 * divideDeliveryAndStorage.
											 * getGuaranteedOutputSearch()
											 */);
		...
		poafZdH.linearInterpolation(input.getAugmentPwerLineNum(), def, def.get(0).getValue(),
				hsSpec.getOutputGuaranteed());
		...
		poafZsH.linearInterpolation(input.getAugmentPwerLineNum(), sef, hsSpec.getPowerInstalled(),
				hsSpec.getOutputGuaranteed());
